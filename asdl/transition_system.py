...
#这个函数根据论文描述，顾名思义，就是生成token，对于一个type，可以生成一个多多个token，并且加入结束符号
# 但这里并没有实现，只有一个基本方法和__repr__输出效果
...
#这个函数标记一个分支的结束，当field有多个选择时，解释用这个函数
# 这里只实现了一个输出效果函数
