...
#     return len(re.findall(gram_sub_str, candidate_str))
#
#
...
#     :return:
#     """
#     candidate_corpus = list(candidate_sentence)
...
#                     # print("            ",gram_candidate_count,curr_gram_deno)
#             # print(' current length %d and gram mole %d and deno %d' % (i+1, curr_gram_mole, curr_gram_deno))
#             if curr_gram_deno != 0 :gram_precisions.append(curr_gram_mole/curr_gram_deno)
#             else: gram_precisions.append(0)
...
if __name__ == '__main__':
    # predict_sentence = ['how']
    # train_sentences = ['how old is the person']
    # bleu_v2_score = my_bleu_v2(predict_sentence, train_sentences, 4, weights=[0.25, 0.25, 0.25, 0.25], mode=0)
    # print('bleu version 2 score is %.2f ' % bleu_v2_score)
    #自己的方法已经和sentence bleu计算结果一致，0.56
    sm = SmoothingFunction()
    score = sentence_bleu([['he', 'is', 'a', 'gay', 'ha']], ['she', 'is', 'a', 'les', 'ha', 'ha'], weights=[0.25] * 2,smoothing_function=sm.method3)
    print(score) #0.5623413251903491

    #这里可以知道，reference的结构应该是[[[]]]，计算时才会将一组reference[[]，[]]找到对应的
    score = corpus_bleu([[['he', 'is', 'a', 'gay', 'ha']]], [['she', 'is', 'a', 'les', 'ha', 'ha']], weights=[0.25] * 2,smoothing_function=sm.method3)
    print(score) #0.35930411196308426
