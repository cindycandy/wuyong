...
class Example(object):
    def __init__(self, src_sent,src_query, tgt_actions, tgt_code, tgt_ast, idx=0, meta=None, related_code = None, relation = None,fixed_relation = None):
        ...
        self.fixed_relation = fixed_relation
        # print("++++++++++++++",relation)
...
class Batch(object):
    def __init__(self, examples, grammar, vocab, copy=True, cuda=False):
        ...
        self.src_query = [e.src_query for e in self.examples]
        ...
        self.copy = copy
        self.cuda = cuda
        self.related_code = [e.related_code for e in self.examples]
        if self.examples[0].relation is None:
            self.relation = [[[0 for i in self.examples] for j in self.examples]]
        self.relation = [e.relation for e in self.examples]
        self.fixed_relation = [e.fixed_relation for e in self.examples]
    ...
    @cached_property
    def relation_var(self):
        # print("___________the 0 1 relation_______________")
        relation = np.array(self.relation)
        max_len = max(len(s) for s in relation)
        #这里注意一下，如何relation是[a,b]的格式，填充后是[c,b+c-a]的格式，会导致relation转tensor错误
        relation = [np.pad(t, ((0, max_len - len(t)),(0, max_len - len(t))), 'constant') for t in relation]
        return relation

    @cached_property
    def position_relation_var(self):
        # print("___________the position relation_______________")
        relation = np.array(self.fixed_relation)
        max_len = max(len(s) for s in relation)
        #这里注意一下，如何relation是[a,b]的格式，填充后是[c,b+c-a]的格式，会导致relation转tensor错误
        relation = [np.pad(t, ((0, max_len - len(t)),(0, max_len - len(t))), 'constant') for t in relation]
        return relation
