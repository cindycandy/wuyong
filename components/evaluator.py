...
class Evaluator(object):
    ...
    def is_hyp_correct(self, example, hyp):
        # print("______________________the correct function_________________")
        # print(hyp.tree,"sosos")
        # print(example.tgt_ast,"gogo")
        return self.transition_system.compare_ast(hyp.tree, example.tgt_ast)

    def get_bleu(self, example, hyp):
        return self.transition_system.compare_ast_bleu(hyp.tree, example.tgt_ast)
            ...
            if hyp_list:
                for hyp_id, hyp in enumerate(hyp_list):
                    try:
                        ...
                        bleu_score = self.get_bleu(example, hyp)
                    except:
                        is_correct = False
