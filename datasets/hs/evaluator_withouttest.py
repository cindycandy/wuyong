...
class HSEvaluator(Evaluator):
    ...
    def evaluate_dataset(self, dataset, decode_results, fast_mode=False, args=None,verbose=True):
        # tokenize_code = tokenize_for_bleu_eval
        import ast
        print ('evaluate_decode_results:: ',dataset.count, len(decode_results))
        # input('>>>')
        ...
        for eid in range(len(dataset)):
            ...
            #运行测试用例
            # if useTest:
            #     path = '/Users/jiechen/Documents/workspace/nl2code/'
            #     cmd = "python3 /Users/jiechen/Documents/workspace/nl2code/hearthbreaker/run_test_one.py -code '%s' -ref_code '%s' -path '%s'"%(code,ref_code,path)
            #     print cmd
            #     os.system(cmd)
            #     with open(path+'/test_result.txt','r') as f_test_result:
            #         cum_test_result[example.raw_id] = f_test_result.readlines()
