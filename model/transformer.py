...
def relative_attention_logits(query, key, relation):
    ...
    if relation is None:
        q_tr_tmatmul_t = torch.zeros(qk_matmul.shape).cuda() #use the gpu
...
def relative_attention_values(weight, value, relation):
    ...
    # print("_____________",weight.shape,value.shape)
    wv_matmul = torch.matmul(weight, value)
    if relation is None:
        w_tr_matmul_t = torch.zeros(wv_matmul.shape).cuda() #use the gpu
    else:
...
class MultiHeadedAttentionWithRelations(nn.Module):
    ...
    def forward(self, query, key, value, relation_k, relation_v, mask=None):
        ...
        # # print("in the encoder,the size:", relation_k.shape)
        # print("the shape of query in muti_attention",query.shape)
        # print("the shape of key in muti_attention",key.shape)
        # print("the shape of key in muti_attention",key.shape)
        # print("the shape of value in muti_attention",value.shape)
        ...
        # print("the shape after linear",query.shape,key.shape,value.shape)
        # 2) Apply attention on all the projected vectors in batch.
...
class Encoder(nn.Module):
    ...
    def forward(self, x, relation, mask):
        ...
        for layer in self.layers:
            # print("in the encoder,the size:",relation.shape)
            x = layer(x, relation, mask)
        return self.norm(x)


# Adapted from The Annotated Transformer
#首先让喂入attSubLayer和feedLayer的x被norm
#要判断上述两个网络，有没有用x+
#如果没有，在上述两个网络，加上dropout和add
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, num_relation_kinds, dropout):
        ...
        self.size = size
        # print("%%%%%%%%%%%%%%%%",num_relation_kinds,self.self_attn.d_k)
    ...
    def forward(self, x, relation, mask):
        ...
        # print("in the encoder layer,the size:", relation.shape)
        if relation is None:
            relation_k = None
            relation_v = None
        else:
            relation_k = self.relation_k_emb(relation)
            relation_v = self.relation_v_emb(relation)
        # x的shape 为 [41,128],与rat的【1.28.256】的区别在于缺少batch层
